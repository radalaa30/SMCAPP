...
class SuiviPreparationImportReaController extends AbstractController
{
    private $repository;
    ...
    #[Route('admin/import/suivi-preparation-rea', name: 'app_import_suivi_preparation_rea')]
    public function import(Request $request): Response
    {
        ...
        if ($request->isMethod('POST') && ($file = $request->files->get('file'))) {
            try {
                ...
                $this->entityManager->createQuery('
                    DELETE FROM App\Entity\SuividupreparationdujourRea s
                ...
                foreach ($rows as $index => $row) {
                    ...
                    if ($codeClient !== '' && (str_starts_with($codeClient, 'CI999') || str_starts_with($codeClient, 'C0'))) {
                        $suivi = new SuividupreparationdujourRea();
                ...
                $totalRecords = $this->entityManager->createQuery('
                    SELECT COUNT(s.id)
                    FROM App\Entity\SuividupreparationdujourRea s
                    WHERE s.updatedAt BETWEEN :startOfDay AND :endOfDay
                ')
                    ->setParameter('startOfDay', $startOfDay)
                    ->setParameter('endOfDay', $endOfDay)
        ...
        return $this->render('suivi_rea/index.html.twig', [
            'message' => $message,
            'error' => $error
        ]);
    }
    ...
    public function getStats(): Response
    {
        try {
            ...
            $totalToday = $this->entityManager->createQuery('
                SELECT COUNT(s.id) FROM App\Entity\SuividupreparationdujourRea s
                WHERE s.updatedAt BETWEEN :startOfDay AND :endOfDay
            ')
                ->setParameter('startOfDay', $startOfDay)
                ->setParameter('endOfDay', $endOfDay)
